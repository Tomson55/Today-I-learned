...
function NewFactForm({ setFacts, setShowForm }) {
  ...
  async function handleSubmit(e) {
    ...
    if (text && isHttpValid(source) && category && textLength <= 200) {
      ...
      if (!error) setFacts((oldfacts) => [newFact[0], ...oldfacts]);
  ...
  return (
    <section>
      <ul className="facts-list">
        {facts.map((fact) => (
          <Factcomp key={fact.id} factObj={fact} setFacts={setFacts} />
        ))}
      </ul>
...
function Factcomp({ factObj, setFacts }) {
